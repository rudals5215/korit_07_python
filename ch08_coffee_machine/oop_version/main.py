...
while is_on:
    ...
    if choice == 'off' :
        ...
        #     continue              # continue가 실행되면 이 이하의 코드라인은 실행되지 않고, while 반복문의 가장 앞 부분으로 돌아가서 다음 반복문 실행

        # 음료 이름을 입력 받은 시점부터의 프로세스를 떠올려서 코드를 작성하셔야만 합니다. 이때 고려해야할 것은 절차지향 방식으로 코딩했을 때의 과정과, 현재 참조해야 하는 파이썬 모듈의 매개변수 차이와 메서드 작동 원리의 차이를 감안해서 코드를 쓸 필요가 있다는 점입니다.
