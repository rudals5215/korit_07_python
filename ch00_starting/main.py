...
type() : 소괄호 내에 있는 데이터(argument)가 어떤 자료형인지 알려주는 명령어
    - JS에서는 typeof가 함수가 아니라 연산자 였습니다.
'''
...
'''
python은 좀 까탈스러워서 Java나 JS 할 때 처럼 맨 처음이 str이면 뒤의 int / float을 알아서 바꿔즈는 짓을 안합니다.

그 때 사용하는 형 변환 함수 (conversion)가 있는데
...
'''
혹시 기억하신다면 JS에서 비슷한 개념을 배웠다느 ㄴ것을 알 겁니다.
console.log(`안녕하세요, 제 이름은 ${name}이고, 나이는 ${age}살 입니다.`);의 python편이라고 생각하시면 됩니다. (20250825.md 참조)
