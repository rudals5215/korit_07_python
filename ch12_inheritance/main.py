...
1. 서브 클래스의 __init__()
    ...
    super(). 에서 super -> 슈퍼 클래스를 의미. 즉 이상의 코드에서 Student 생성자를 호출하려면 super().__init__(name)에 의해서 슈퍼 클래스인 Person의 생성자가 먼저 호출되면 '슈퍼 클래스의 객체가 생성됩니다.'
    이후에 슈퍼 클래스에서 생성된 인스턴스 변수인 name이 서브 클래스로 전달되고, 이후에 서브 클래스에서 school 인스턴스 변수를 선언 및 초기화하여 저장하면서 서브 클래스의 인스턴스가 생성 됩니다.
    
    : 생성자를 호출했다면 -> 객체가 생성되었다고 봐야하기 떄문에 / 부모 클래스의 인스턴스와 자식 클래스의 인스턴스가 있다고 봐도 무방합니다. -> 그런데 별개의 객체냐고 물으면 그건 또 그럴 때도 있고 아닐 때도 있습니다.
    
2. 서브 클래스의 인스턴스 자료형
    슈퍼 클래스의 객체는 슈퍼 클래스의 인스턴스
    하지만 서브 클래스의 인스턴스는 서브 클래스의 인스턴스이면서 동시에 슈퍼 클래스의 인스턴스
    Student 클래스의 객체는 Student의 인스턴스 이면서 Person의 인스턴스
...
#
#     def add_oil(self, oil):                 # 에서드 # 1
#         if oil <= 0:                        # 매개변수 oil이 0 이하라면
#             print('0보다 작은 값으로 충전할 수 없습니다.')
#             return                          # 매서드 종료
#         self.oil += oil                     #
...
#         super().add_oil(oil)
#         print(f'기름을 {oil} 주유했습니다.')
#         if self.oil > Hybrid.max_oil:
#             self.oil = Hybrid.max_oil
#             print('기름이 가득 찼습니다.')
#
...
#             print('과충전 입니다.')
#
#         print(f'전기를 {amount} 충전했습니다.')
#
#     # 고유 메서드 정의했는데 부모 메서드를 가지고 오고 추가함. car_info() override해서 재정의도 가능
...
3. Shape 클래스를 상속 받는 서브 클래스 Rectangle을 정의하시오
    ...
    - area() 메서드를 정의하여 사각형의 넓이를 계산하고 return할 것 -> call3()
        (넓이 = 너비 * 높이)
