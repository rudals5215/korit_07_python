'''
1. 클래스 변수 vs. 인스턴스 변수
    인스턴스 변수 :ㅣ인스턴스 마다 서로 다른 값을 가지는 변수
    클래스 변수 : 모든 인스턴스가 동일한 값을 지니는 변수 (Java에서는 정적 변수)

    인스턴스 변수
        목적 - 인스턴스마다 서로 다른 값을 저정
        접근 방식 - 인스턴스 접근 (o)
                 - 클래스 접근 (x)
...
'''
객체명.클래스변수 를 통해서 클래스 변수에 접근이 가능하긴 하지만, 클래스 내부의 코드를 들여다 보기 전까지는 country라는 변수가 인스턴스 변수인지 클래스 변수인지 알 방법이 없다는 문제가 있습니다.

이상을 이유로 클래스 변수를 확인하고자 할 때는 객체명.클래스변수명 보다는
클래스명.클래스변수명 을 총해서 참조하는 것이 권장됩니다.
...
man2 = Korean2()
man2.trip('일본')
# 클래스 변수와 마찬가지로 객체명.클래스메서드명() 으로 호출이 가능하기는 하지만 마찬가지로 이게 인스턴스 메서드인지 알 바가 아니기 때문에 클래스메서드 호출할 때는
# 클래스명.클래스메서드명() 으로 하는 것이 권장됩니다.

'''
특징 : 
    1) 인스턴스 / 클래스로 호출 가능 -> 하지만 클래스로 호출하도록 권장
    2) 생성된 인스턴스가 ㅇ벗어도 호출 가능(클래스로 호출하면 되니까)
...
3. 정적 메서드 (static method)
    ...
    3) @staticmethod 데코레이터를 표기하고 작성 -> 클래스 메서드와의 공통점 # 3
    4) 반드시 작성해야 할 매개변수 (self / cls) 가 없음 -> 클래스 메서드와의 차이점 # 2

이상을 토대로, 정적 메서드는 self / cls를 둘 다 사용하지 않기 떄문에 인스턴스 / 클래스 변수를 모두 사용하지 않는 메서드를 정의하는 경우에 적합합니다. 정적 메서드 자체는 클래스에 소속되어있지만 인스턴스에는 영향을 주지도 않고 받지도 않습니다.
...
가방을 만들 때 마다 현재 만들어진 가방이 몇 개인지 계산할 수 있는 Bag 클래스를 정의 할겁니다.
'''
class Bag :
    # 클래스 변수의 선언 및 초기화
    count = 0

    def __init__(self):    # 생성자 호출 및 인스턴스 변수들 정의할 용도. 그럼 생성자도 인스턴스 변수라고 할 수 있곘네요(self가 있으니까)
...
1. Shop 클래스는 다음과 같은 변수를 가지고 있다.
    total : 가게 전체 매출액
    menu_list : {메뉴명 : 가격}으로이루어진 dictionary 요소를 가진 list
    
    menu_list = [{'떡볶이':3000},{'순대' : 4000},{'튀김' : 500}, {'김밥':2000}]
    
2. 매출이 생기면 다음과 같은 방식으로 판매량을 작성합니다.
Shop.sales('떡볶이', 1)            # 떡볶이 을(를) 1개  판매
Shop.sales('김밥', 2)              # 떡볶이 을(를) 1개  판매
Shop.sales('튀김', 5)              # 떡볶이 을(를) 1개  판매
...
class Shop :
    ...
    @classmethod
    def sales(cls, food, num):
        for menu_dict in cls.menu_list:
            if food in menu_dict:       # 딕셔너리 내에 pair 하나 밖에 없는 상태인데, 있으면 이하의 코드라인이 실행되고 아니면 넘어갈겁니다. 그러면 다음 반복으로 가겠네요.
                # in -> element를 기준으로 해야 하기 때문에 dictionary의 element중 'key'를 기준
                # 애초에 key 없으면 value를 조회 못하는게 dictionary의 특징 중 하나
                cls.total += menu_dict[food] * num
                print(f'{food} 을(를) {num}개 판매')

    @classmethod
    def sales2(cls, food, num):
        if food in cls.menu_dict:
            cls.total += (num * cls.menu_dict[food])    # 그러면 굳이 반복문 다 돌려서 일치하는 key가 있는지 확인할 필요가 없습니다.
            print(f'{food} 을(를) {num}개 판매')
