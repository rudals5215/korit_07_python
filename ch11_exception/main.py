...
#     result = b / a
'''
근데 우리는 어제 사용해봤었습니다. coffe_machine에서.
if drink == None : 
    이라는 방식으로 사용했었죠.
...
|  5   |       EOFError      |     파일에서 더이상 읽어 들일 데이터가 없을 때     |
...
다음은 사용자가 이력한 키를 정수로 반올림해서 다시 저장하는 프로그램입니다.
...
'''
    1) 특정 예외만 처리하는 방식
        try / except 문을 사용하면 기본적으로 예외의 종류에 상관없이 모든 예외가 처리됨. 하지만 이상에서 본 것 처럼, 0으로 나누는 경우와, str 자료형을 실수로 바꾸고자 하는 경우에 서로 다른 메시지를 출력해줄 수 있다면, 개발자에게 예외를 처리할 수 있을만한 추가적인 정보를 제공할 수 있음.
...
#     b = input('나누어지는 수(정수)를 입력하세요 >>> ')
#     print(f'b / a = {b/a}')
# except ZeroDivisionError :
#     print('0으로 나눌 수 없습니다.')
'''
        거의 모든 예외 Exception 클래스의 서브 클래스에 해당함. 따라서 모든 예외는 EXception의 인스턴스가 됩니다, 다음과 같으 except의 마지막에 Exception을 명시하면 예상하지 못한 예외들도 웬만하면 처리 됩니다.
...
5. 강제로 예외를 발생시키기
    ...
    컴퓨터 상으로는 (그리고 파이썬 상으로는) -1000이 정수이기 때문에 예외가 발생하지 않습니다.(그래서 우리는 0미만 200 초과를 조건문으로 처리하는 것을 학습했었습니다)
...
'''
이상은 특정 예외가 아니라 258 번으로 넘어가기만 하면 바로 예외가 발생해버립니다.
즉 age에 가능한 정수값을 넣더라도 예외가 발생하기 때문에 단독으로 처리가 안되는거죠.
...
사용자의 점수를 0 이상 100 이하로 입력 받아서 점수가 80점 이상이면 합격, 아니면 불합격을 출력하는 프로그램을 작성하는데, 0 이상 100 이하의 유효한 값이 아니라면 예외를 발생시키고
'점수는 0이상 100 이하입니다.'라는 예외 메시지를 출력하시오 -> 사용자 정의 예외 클래스를 정의해야 하겠죠.
ScoreOutRangeError 클래스를 정의해서 사용하겠습니다.

입력 받는데 예를 들어 백점이라고 입력하면 '점수는 숫자로 입력해야 합니다.' 라는 메시지를 출력하세요.
...
'''
# 0점 미만에 대한 사용자 정의 클래스를 추가 NegativeScoreRage -> 점수는 0 이상 100 이하입니다.
...
#     if score > 100 :
#         raise ScoreOutRangeError('점수는 0 이상 100 이하입니다.')
#     elif  score < 0 :
#         raise NegativeScoreRange('점수는 0 이상 100 이하입니다.')
#     score = int(score)
#     if score > 80 :
#         print('합격 입니다.')
#     else :
...
만약 사용자가 0을 입력하거나 숫자가 아닌 값을 입력하면적절한 예외 메시지를출력하시오.

지시 사항
1. 사용자로 부터 숫자를 입력 받는다.
...
7. 프로그램 종료 메시지를 출력한다.
8. 마이너스 인덱스는 적용시키지 않는다. -> 사용자 정의 예외 클래스를 통해서 적용. -> NegativeNumIndexError라고 이름 짓고 처리
'''
...
#     print(f'입력받은 값의 list 숫자는 {my_list[num]}입니다.')
#
# except NegativeNumIndexError as e:
#     print('0 이상의 숫자를 입력하세요.')
#     print(e)
# except IndexError as e:
#     print('인덱스 범위 안쪽의 숫자를 입력하세요.')
#     print(e)
# except ValueError as e:
#     print('정수를 입력하세요.')
#     print(e)
...
# # getatter()의 두 번째 argument는 인스턴스 변수명을 받습니다. -> 그 데이터를 srt으로 받습니다.
...
# try :
#     value = input('속성값을 입력하세요.')
#     result = getattr(person1,value)
