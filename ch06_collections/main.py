...
for element in li4:
    ...
    n += 1
print(li4)
# 사실상 list의 element들에게 동일한 연산을 일괄적으로 적용하는 method가 있기 때문에 추후 설명할 예정. 백엔드에서 보다 프론트에서 더 많이 쓰여서 예시는 이후 수업
'''
    2. tuple 튜플
...
마찬가지로 tuple의 특성 상 element의 추가 / 수정 / 삭제가 불가능하기 때문에 수업할 일이 없겠네요.
'''
...
'''
    se = {} 형태로 비어있는 set을 생성했을 경우 se는 사실 <class 'dict'>로 나옵니다. 아직 배우지 않은 dictionary 자료형으로 생성됩니다. 그래서 비어있는 set를 생성하기 위해서는 반드시 # 2 방식으로 만들어줘야 합니다.
'''
se2 = set({})       # 이렇게요
print(type(se2))    # 결과값 : <class 'set'>
'''
    list / tuple은 index가 존재한다고 했습니다. 이 두 개를 sequence라고 하고, set / dictionary 의 경우에는 index가 없어서 비시퀀스라는 표현을 씁니다. 슬라이싱이 없겠네요.
...
# 향상된 for문으로 element를 추출할 수 있습니다. 순서를 보장 못해서 그렇지
for element in se3:
    print(element)
...
'''
    전에 수업했던 것처럼 189번 라인까지 지금 key - value pair가 있는데 189번 라인에 콤마치고 엔터치고 키 - 값 입력하면 번거로우니까 맨 마지막 property에 콤마 찍어주는게 매너라고 합니다.
...
# 매우 중요해서 한 번 더 말하는거 dictionary/JS Object에서 향상된 for문으로 반복문 돌리면 key가 빠져나옵니다.
...
# key들만 뽑아서 list를 만든다든지 / value들만 뽑아서 list를 만들고 싶다면 형변홚마수를 사용해야함.
